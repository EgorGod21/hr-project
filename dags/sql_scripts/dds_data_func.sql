...
-- после выполнения следующих функций "битые" данные будут хранится в схеме dds_er

-- заполняет таблицу в dds слое, у которой 4 признака (id, название, активность, "Дата изм.")
CREATE OR REPLACE FUNCTION _insert_4_col_dds(tab_name text)
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
    EXECUTE format(
        'INSERT INTO dds.%I
    ...
    EXECUTE format(
        'INSERT INTO dds_er.%I
			SELECT 
			    ...
			    t1.активность::TEXT,
			    t1."Дата изм."::TEXT,
                CURRENT_TIMESTAMP(0) AS "Дата создания"
			FROM ods.%I t1
			WHERE NOT EXISTS (
			        SELECT 1 
			        FROM dds.%I t2 
...
CREATE OR REPLACE FUNCTION insert_4_col_all_tables_dds()
RETURNS void LANGUAGE plpgsql AS $$
DECLARE
    tab_name text;
...
BEGIN
    ...
    LOOP
        PERFORM _insert_4_col_dds(tab_name);
    END LOOP;
END;
$$;
...
BEGIN
    ...
    EXECUTE format(
        ...
        main_table, main_table, main_table
    );
END;
$$;
...
-- заполняет таблицу слоя dds, у которой 7 признаков (id, "User ID", активность, "Дата изм.", дата и 2 признака
-- зависят от заполняемой таблицы)
CREATE OR REPLACE FUNCTION insert_7_col_user_id_int_dds(
...
END)
	...
	INSERT INTO dds_er.языки_пользователей
        SELECT 
            t1.id,
            t1.название::TEXT,
            t1.активность::TEXT,
	    ...
	    AND NOT EXISTS (
			SELECT 1
			FROM dds_er.языки_пользователей t3
			WHERE t1.id = t3.id
		);
END;
$$;
...
BEGIN
	...
	INSERT INTO dds_er.сертификаты_пользователей
        SELECT 
            t1.id,
            t1."User ID"::TEXT,
            t1.активность::TEXT,
	    ...
	    AND NOT EXISTS (
			SELECT 1
			FROM dds_er.сертификаты_пользователей t3
			WHERE t1.id = t3.id
		);
END;
$$;
...
BEGIN
	...
	INSERT INTO dds_er.образование_пользователей
        SELECT 
            t1.id,
            t1."User ID"::TEXT,
            t1.активность::TEXT,
	    ...
	    AND NOT EXISTS (
			SELECT 1
			FROM dds_er.образование_пользователей t3
			WHERE t1.id = t3.id
		);
END;
$$;